...
const EMPTY = { title: '', titleFr: '', description: '', descriptionFr: '', category: 'rental', icon: 'court', features: '', featuresFr: '', order: 0, isActive: true };
...
export default function ProgramsPage() {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => { if (k !== 'features' && k !== 'featuresFr') fd.append(k, v); });
      fd.append('features', JSON.stringify(form.features.split('\n').map(f => f.trim()).filter(Boolean)));
      fd.append('featuresFr', JSON.stringify((form.featuresFr || '').split('\n').map(f => f.trim()).filter(Boolean)));
            ...
            <form onSubmit={save} className="modal-form">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <textarea value={form.descriptionFr || ''} onChange={e => setForm(f => ({ ...f, descriptionFr: e.target.value }))} rows={3} placeholder="French translation (optional)" />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                ...
                <div className="form-group">
                  <label>FR: Features (one per line)</label>
                  <textarea value={form.featuresFr || ''} onChange={e => setForm(f => ({ ...f, featuresFr: e.target.value }))} rows={4} placeholder="French translation (optional)" />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
